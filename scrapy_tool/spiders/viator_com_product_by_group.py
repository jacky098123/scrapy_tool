...
#        url     = "http://www.viator.com" + self.ToString(url)
    ...
    def start_requests(self):
        request_list    = []
        sql = "select id,href from viator_ttd_group where flag='init' and source = 'proc_gen' "
    ...
    def parse(self, response):
        ...
        for _selector in tour_selector_lst:
            ...
            sql = "insert into viator_ttd_product(ttdid,href,title,description,location,price) values(%s,%s,%s,%s,%s,%s)"
            self.db_conn.Execute(sql, [kx_args['id'],href,title,description,location,price])

        if len(tour_selector_lst) > 0:
            self.db_conn.Execute("update viator_ttd_group set flag='done' where id=%s", [kx_args['id'],])
        else:
            self.db_conn.Execute("update viator_ttd_group set flag='passed' where id=%s", [kx_args['id'],])
